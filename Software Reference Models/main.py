...
def analyseTestSet(loader, hiddenWeights, outputWeights, hiddenBias, outputBias, activationName, bits):
    correct = 0
    total = 0
    wrongIndices = []
    wrongPredictions = []
    ...
    for batch_idx, (data, targets) in enumerate(loader):
        ...
        for i in range(len(targets)):
            if predictions[i] != targets[i]:
                wrongIndices.append(total + i)
                wrongPredictions.append(predictions[i].item())
                wrongTargets.append(targets[i].item())
    ...
    print(f"Wrong predictions: {len(wrongIndices)}")
    print(f"Wrong indices: {wrongIndices}")
    print(f"Wrong predictions vs targets: {list(zip(wrongPredictions, wrongTargets))}")
    
    # Check if it's always the same samples
    if len(wrongIndices) > 0:
        print(f"Consistent error pattern: {len(set(wrongIndices)) == len(wrongIndices)}")
...
def exportWeights(hiddenWeight, outputWeights, hiddenBias, outputBias, activationName, testAccuracy, bits, filenamePrefix=""):
    ...
    weights = {
        ...
        'nodeArchitecture': f"{NumInputNodes}-{NumHiddenNodes}-{NumOutputNodes}",
        'bit resolution': bits,
    ...
    filename = f"{filenamePrefix}NN_{NumHiddenNodes}hidden_{bits}bit_{activationName}.json"
    ...
    for activationName in ['ELU', 'ReLu', 'Sigmoid']:
        ...
        for numHiddenNodes in HiddenNodeCount:
            ...
            for bits in bitResolutions:
                ...
                testAccuracies = evaluateModel(testLoader, hiddenWeights, outputWeights, hiddenBias, outputBias, activationName, bits)

                exportWeights(hiddenWeights, outputWeights, hiddenBias, outputBias, activationName, testAccuracies, bits)
